...
本地运行评测后，`eval_out/` 下的产物只包含**聚合指标**（Recall / MRR / 延迟 / 行数），
不含任何语料正文。该目录已被 Git 忽略。评测过程用的临时 SQLite 库建在
`eval_out/_work/`，跑完自动删除
...
上述规则对应 `docs/API_CONTRACT.md` 第 7 节记录的赛事数据义务；如上游规则更新，
以当前官方文档为准。
