...
本仓库以首次提交身份参加排行榜，尚未获得官方评测分数。本文的合成指标只用于
...
| `content_timestamp_prefix` | 返回证据前加 `[<事件时间>]` 前缀（日级，不改原文不改排序），供答案模型在时间问题上使用；**v1.2.1 起仅对含时间上下文（时间/日期意图、显式日期、月份名、时长）的查询生效** | 端到端（DeepSeek 官网 flash 答案 + pro 评判，297 项分层抽样）：**0.6229 vs 0.5724（+0.0505，p=0.018）**；gated 版配对 +1.35pp、修复非时间类回退（多跳类 +0.06） |

> v1.2.1 门控说明：无条件前缀在非时间类问题小幅回退（locomo cat4 −0.008 / cat5 −0.030），
> 窄门控（仅时间/日期意图）又漏掉 23% 的日期类问题（cat2 −0.063）；扩展为
> `has_temporal_context`（意图 + 显式日期 + 月份名 + 时长）后漏检降至 1/48，
> 配对消融：总体 +1.35pp（非显著，两轮均非负）、多跳类 +0.06、日期类回到噪声内。
