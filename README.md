...
> v1.1 尚未参加官方复评，不能用本地 MRR 推算官方综合分。详见 [证据状态表](#8-证据状态表)。

**版本与成绩必须分开看**：v1.0 已进入首期文本记忆学术方法公开榜，系统名
`FlowGrid_AML_Retriever`，公开快照为**第 8 名、综合分 43.98**；v1.1 是本分支的后续本地候选，
尚未提交官方复评。公开榜见 [Agent Memory Leaderboard](https://agentmemories.ai/leaderboard/)。
...
# 一键总验收：环境探测 + 146 项单测 + CLI 自检 + 31 项 HTTP 契约 smoke
...
3. **重排**：相对新近度、邻接上下文、provenance、当前/历史状态冲突提示；
   v1.1 只在“时间意图 + 成对近重复 + 显式更新语义”同时成立时做保守覆写重排，
   把旧值降权但**绝不删除**。
...
## 6. v1.1 消融结论（纯合成，medium/mixed，3 seeds，top_k=100）

| 档位 | 角色 | Recall@20 mean (min–max) | Recall@100 | MRR mean (min–max) |
| --- | --- | --- | --- | --- |
| `L5_plus_weighted_rrf` | v1.0 默认 / 直接基线 | 0.9948 (0.9870–1.0000) | 1.0000 | 0.6728 (0.6631–0.6791) |
| `L8_supersession_ctrl` | 无保护 4/1 对照 | 0.9939 (0.9844–1.0000) | 1.0000 | 0.6780 (0.6680–0.6834) |
| **`L9_guarded_supersession`** | **v1.1 默认** | **0.9948 (0.9870–1.0000)** | **1.0000** | **0.6948 (0.6854–0.7004)** |

诚实记账：

- `L9` 的保守 4/1 权重在三个 seed 上都保持 L5 的 Recall@20，同时 MRR 每个 seed 都上升；
  更激进的 14/4、18/6 虽多约 0.009 MRR，却会让一个 seed 的 Recall@20 下降，故拒绝作为默认。
- 无保护 L8 是不安全对照：它改善部分时序题且总 MRR 小涨，但一个 seed 的 Recall@20
  从 0.9870 降到 0.9844，并轻伤 `knowledge_update`，不能与 L9 混为一谈。
- `L10_preference_ctrl` 在 v11 偏好代理题上把 MRR 从 0.5834 提到 1.0000，
  但场景宽度不足，仍默认关闭。
- `temporal × paraphrase` 仍是短板；L9 有明显改善但没有解决语义改写上限。
...
| v1.0 首期公开榜 | **observed（官方公开榜快照）** | 文本记忆学术方法榜第 8，综合分 43.98；与 v1.1 本地实验分开陈述 |
| v1.1 在官方数据/榜单上的真实表现 | **unknown** | 尚未提交官方复评；不得由本地 MRR 外推 |
| Dockerfile 能否构建成功 | **confirmed（v1.1 本机）** | Engine 29.6.2：镜像构建、FTS5、146 tests、31/31 smoke、默认 CMD 的 health/Add/Search 均通过 |
| `gpt-4o-mini` Full 自助门控对代码托管路线的适用性 | **needs recheck** | 官网自助 Full 清单有该项，但 v1.0 已获公开结果；v1.1 提交前向主办方复核适用路径 |
| v1.1 报名 / 部署 / 正式复评 | **not executed** | 本轮只做本地开发、评测与验收 |
...
1. **改写类时间查询仍是短板**。受保护覆写可改善成对更新，但纯确定性词法仍无法覆盖所有语义改写。
2. **旧值泄漏不为 0**。原始消息必须全量保留，系统只降权与标注，不删除旧值。
...
Dockerfile        本机已构建与运行验证（Docker Engine 29.6.2）
config.example.json
...
本轮没有申请新 Key、没有部署、没有提交 v1.1 正式评测、没有发邮件、没有产生费用，
也没有接触真实用户数据或赛事私有数据。v1.0 首期公开成绩是历史事实，不能冒充 v1.1 的验证。
